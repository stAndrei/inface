...
| Функция | Описание |
|--------|---------|
| Таймлайн в menu bar | Встречи на день, колонки при пересечениях; пока грузятся — «Загружаем…» |
...
| Подключение к встрече | Детект ссылок; **ChatZone** `/meetzone/{uuid}` открывается как звонок `/meet/{uuid}` в ChatZone.app |
